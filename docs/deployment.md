...
- `MUTATION_AUDIT_CORRELATION_SECRET` is a server-only production value of at
  least 32 random characters and exactly matches the protected PostgreSQL
  `app.settings.mutation_audit_correlation_secret` setting. Verify equality
  through the approved secret-management/database procedure without printing
  either value. A missing or mismatched value is a deployment failure: signed
...
7. Through the approved secret-management procedure, configure the protected
   database mutation-correlation setting to match the server-only application
   secret. Do not place either value in migration SQL, source, command history
   or release evidence.
8. Through the approved tracked facility, apply every pending file discovered
   from `supabase/migrations/`, in filename order.
9. Compare the facility's applied-version report with the complete migration
   directory at the release commit. Stop if any version is absent or failed.
10. Complete the non-destructive database and staging checks below, including
...
11. Deploy or start the exact validated application artifact.
12. Complete post-deploy health, authentication and read-path verification.
...
steps 5–10 remain a blocking precondition.
...
- the append-only mutation audit table and triggers exist, ordinary application
  roles have no direct access, and the approved staging proof shows signed
  application correlation fails closed when invalid;
