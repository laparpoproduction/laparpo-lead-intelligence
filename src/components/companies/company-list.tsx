...
export function CompanyList({ companies, canDelete }: CompanyListProps) {
  return (
    <div className="overflow-hidden rounded-2xl border border-zinc-200 bg-white shadow-sm">
        ...
        <table className="w-full min-w-[980px] border-collapse text-left">
          ...
          <tbody className="divide-y divide-zinc-100">
            {companies.map((company) => (
              <tr className="transition hover:bg-zinc-50/70" key={company.id}>
                ...
                <td className="px-5 py-4"><CompanyActions canDelete={canDelete} company={company} /></td>
              </tr>
            ))}
          </tbody>
        </table>
      ...
      <ul className="divide-y divide-zinc-100 md:hidden" aria-label="Companies">
        {companies.map((company) => (
          <li className="p-5" key={company.id}>
            ...
            <div className="mt-5 border-t border-zinc-100 pt-4"><CompanyActions canDelete={canDelete} company={company} /></div>
          </li>
        ))}
      </ul>
    </div>
